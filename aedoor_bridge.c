 ...
 *
 * This module wraps the low-level door open/close, caller-context fetch, and
...
/* Caller context and session setup */
...
void aedoor_clear_screen(struct aedoor_context *context)
{
   ...
   * sequence through AEDoor to force a real repaint on ANSI callers.
   */
  SetDT(context->diface, BB_NONSTOPTEXT, "1");
